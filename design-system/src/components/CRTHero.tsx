...
export function CRTHero({
  ...
  return (
    <div className="relative border-b-2 border-navy-900 overflow-hidden crt-screen">
      ...
      <div className="container mx-auto px-6 py-24 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            {badge && (
              <div>
                <Badge variant="outlined" size="md" className="crt-badge border-navy-900 text-navy-900">
                  {badge}
                </Badge>
              </div>
            )}
            ...
            <div className="space-y-4">
              <h1 className="text-navy-900 crt-text" style={{ fontSize: '4.5rem', lineHeight: '1' }}>
                {title}
              </h1>
              {subtitle && (
                <div className="crt-text text-2xl tracking-wider text-navy-600">
                  {subtitle}
                </div>
              )}
            </div>
            ...
            {description && (
              <p className="text-lg leading-relaxed text-navy-700 max-w-xl">
                {description}
              </p>
            )}
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                {primaryCta && (
                  <Button
                    ...
                    className="bg-navy-900 text-white border-2 border-navy-900 hover:bg-navy-700 hover:border-navy-700"
                  >
                    {primaryCta.label}
                  </Button>
                )}
                {secondaryCta && (
                  <Button
                    ...
                    className="text-navy-900 hover:bg-navy-100"
                  >
                    {secondaryCta.label}
                  </Button>
                )}
            ...
            {/* Technical metadata */}
            <div className="pt-8 border-t border-navy-300">
              <div className="grid grid-cols-2 gap-6 text-sm font-mono">
                <div>
                  <div className="text-navy-500 uppercase text-xs mb-1">Status</div>
                  <div className="crt-text text-navy-900">OPERATIONAL</div>
                </div>
                <div>
                  <div className="text-navy-500 uppercase text-xs mb-1">Location</div>
                  <div className="crt-text text-navy-900">GLOBAL GRID</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Bottom metadata bar */}
        <div className="mt-16 pt-8 border-t border-navy-300 font-mono text-xs text-navy-600 uppercase tracking-wider">
